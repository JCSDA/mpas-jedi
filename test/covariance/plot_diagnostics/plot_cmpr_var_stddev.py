...
#for current exp
file = 'mpas.stddev.2018-04-15_00.00.00.nc'
f_exp = Dataset(file, "r", format="NETCDF4")

#for comparison
file = '../../20220111/CMAT_00/mpas.stddev.2018-04-15_00.00.00.nc'
...
for var in list_variables:
   ...
   if var == 'surface_pressure' :
      ...
      print( '*** For '+var+': exp / ref = ', np.mean(data_exp, axis=0), np.mean(data_ref, axis=0) ) # [own] unit
   else:
      data_exp = np.array( f_exp.variables[var][0,:,:] ) # (nl0, nc0)
      data_ref = np.array( f_ref.variables[var][0,:,:] ) # (nl0, nc0)
   ...
   if var == 'stream_function' :
      vdata1_exp=np.mean(data_exp, axis=0) # [own] unit
      vdata1_ref=np.mean(data_ref, axis=0) # [own] unit
   if var == 'velocity_potential' :
      vdata2_exp=np.mean(data_exp, axis=0) # [own] unit
      vdata2_ref=np.mean(data_ref, axis=0) # [own] unit
   if var == 'temperature' :
      vdata3_exp=np.mean(data_exp, axis=0) # [own] unit
      vdata3_ref=np.mean(data_ref, axis=0) # [own] unit
   if var == 'spechum' :
      vdata4_exp=np.mean(data_exp, axis=0) # [own] unit
      vdata4_ref=np.mean(data_ref, axis=0) # [own] unit
